...
### 3.7.1 Time-depth grid
为了精确描述作物生育季节土壤剖面的水分留存、运动和作物吸收，AquaCrop将所研究的剖面和计算时间均分解为小段，如图3.6a所示。因此一维垂向的水分运动和作物吸水可以采用有限差分技术进行求解。$\Delta z$和$\Delta t$分别为空间和时间离散。以下采用$i$表示空间离散，$j$表示时间离散。

<img src=".\Figs\Fig3.7a.png" width="500px" />

在AquaCrop中，时间步长$\Delta t$固定为1天，空间步长$\Delta z$默认为0.1 m。土壤剖面默认被划分为12层。土壤水力参数由该层所属的土质决定。如果根系层太深，那么则会调整$\Delta z$以包含全部的根系层。$\Delta z$并不固定，从上至下按照指数规律增加。因此发生在地表附近的入渗、土壤蒸发和植物蒸腾可以有较好的计算精度。可以从程序中设定土壤剖面所包含的层数和$\Delta z$。

### 3.7.2 Calculation scheme
在AquaCrop中，偏微分方程被一系列有限差分方程所取代，并且方程自变量为$\theta$。模拟的第一步为排水的计算，随后是水分入渗过程（减去地表径流），再之后是在毛细作用下从地下水位向上的运动，最后计算土壤蒸发和作物蒸腾消耗的剖面水量。
